...
static int msg_hello(char *nick, char *h, struct userrec *u, char *p)
{
  ...
  if (common) {
    dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_SALUT2A);
    dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_SALUT2B);
  }
  if (make_userfile) {
    dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_INITOWNER1);
...
static int msg_pass(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (!u_pass_match(u, "-") && !par[0]) {
    dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_EXISTPASS);
    return 1;
  }
  if (par[0]) {
    if (!u_pass_match(u, old)) {
      dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_FAILPASS);
      return 1;
    }
    new = newsplit(&par);
  } else
...
static int msg_ident(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (!u2) {
    ...
    if (u_pass_match(u2, "-")) {
      ...
      maskhost(s, s1);
      dprintf(DP_HELP, ":%s NOTICE %s :%s: %s\n", botname,nick, IRC_ADDHOSTMASK, s1);
...
static int msg_addhost(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (!par[0]) {
    if (!quiet_reject)
      dprintf(DP_HELP, ":%s NOTICE %s :You must supply a hostmask\n", botname,nick);
...
static int msg_info(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (par[0]) {
    ...
    if (par[0] == '@')
      par++;
    dprintf(DP_HELP, ":%s NOTICE %s :%s %s\n", botname,nick, IRC_FIELDCHANGED, par);
    if (chname) {
  ...
  if (p && p[0]) {
    ...
    if (chname)
      dprintf(DP_HELP, ":%s NOTICE %s :%s %s.\n", botname,nick, IRC_NOINFOON, chname);
    else
      dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_NOINFO);
  }
  return 1;
}
...
static int msg_who(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (!par[0]) {
    dprintf(DP_HELP, ":%s NOTICE %s :%s: /msg %s who <channel>\n", botname,nick,
            MISC_USAGE, botname);
    return 0;
  }
  ...
  for (m = chan->channel.member; m && m->nick[0]; m = m->next) {
    ...
    if (info && info[0])
      ...
      if (match_my_nick(m->nick))
        ...
        if (i > 400) {
          dprintf(DP_HELP, ":%s NOTICE %s :No info: %s\n", botname,nick, also);
          i = 0;
          also[0] = 0;
        }
      }
...
static int msg_whois(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (!par[0]) {
    dprintf(DP_HELP, ":%s NOTICE %s :%s: /msg %s whois <handle>\n", botname,nick,
            MISC_USAGE, botname);
    return 0;
  }
  ...
  if (!u2) {
    ...
    for (chan = chanset; chan && !ok; chan = chan->next) {
      ...
      if (m) {
        ...
        if (u2) {
          ok = 1;
          dprintf(DP_HELP, ":%s NOTICE %s :[%s] AKA '%s':\n", botname,nick,
                  par, u2->handle);
        }
      }
    }
    if (!ok) {
      dprintf(DP_HELP, ":%s NOTICE %s :[%s] %s\n", botname,nick, par, USERF_NOUSERREC);
      return 1;
    }
  }
  ...
  if (s2 && s2[0] && !(u2->flags & USER_BOT))
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] %s\n", botname,nick, u2->handle, s2);
  for (xk = get_user(&USERENTRY_XTRA, u2); xk; xk = xk->next)
    if (!egg_strcasecmp(xk->key, "EMAIL"))
      dprintf(DP_HELP, ":%s NOTICE %s :[%s] E-mail: %s\n", botname,nick, u2->handle,
              xk->data);
  ok = 0;
  for (chan = chanset; chan; chan = chan->next) {
    if (hand_on_chan(chan, u2)) {
      egg_snprintf(s1, sizeof s1, ":%s NOTICE %s :[%s] %s: %s.", botname,nick, u2->handle,
      ...
      if (cr && (cr->laston > tt) && (!channel_hidden(chan) ||
        ...
        ok = 1;
        egg_snprintf(s1, sizeof s1, ":%s NOTICE %s :[%s] %s %s on %s", botname,nick,
                     u2->handle, IRC_LASTSEENAT, s, chan->dname);
      }
    }
  }
  if (!ok)
    egg_snprintf(s1, sizeof s1, ":%s NOTICE %s :[%s] %s", botname,nick, u2->handle,
                 IRC_NEVERJOINED);
  dprintf(DP_HELP, "%s\n", s1);
  if (u2->flags & USER_BOT)
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] Status: bot\n", botname,nick, u2->handle);
  else if (u2->flags & USER_OWNER)
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] Status: global owner\n", botname,nick, u2->handle);
  else if (u2->flags & USER_MASTER)
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] Status: global master\n", botname,nick, u2->handle);
  else if (u2->flags & USER_BOTMAST)
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] Status: botnet master\n", botname,nick, u2->handle);
  else if (u2->flags & USER_OP)
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] Status: global op\n", botname,nick, u2->handle);
  else if (u2->flags & USER_VOICE)
    dprintf(DP_HELP, ":%s NOTICE %s :[%s] Status: global voice\n", botname,nick, u2->handle);
  return 1;
}
  ...
  if (!u) {
    if (!quiet_reject) {
      if (!learn_users)
        ...
        dprintf(DP_HELP, ":%s NOTICE %s :%s\n", botname,nick, IRC_DONTKNOWYOU);
        dprintf(DP_HELP, ":%s NOTICE %s :/MSG %s hello\n", botname,nick, botname);
      }
    }
    return 0;
  }
  ...
  if (u_pass_match(u, pass)) {
    ...
    if (!u_pass_match(u, "-")) {
      ...
      if (!channel_active(chan)) {
        dprintf(DP_HELP, ":%s NOTICE %s :%s: %s\n", botname,nick, par, IRC_NOTONCHAN);
        return 1;
      }
      chan = findchan_by_dname(par);
      if (chan && channel_active(chan)) {
        ...
        if (chan_op(fr) || chan_halfop(fr) || (glob_op(fr) && !chan_deop(fr)) ||
          ...
          if (chan->channel.key[0]) {
            dprintf(DP_SERVER, ":%s NOTICE %s :%s: key is %s\n", botname,nick, par,
                    chan->channel.key);
...
static int msg_status(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  dprintf(DP_HELP, ":%s NOTICE %s :%s %s.\n", botname,nick, MISC_ONLINEFOR, s);

  if (admin[0])
    dprintf(DP_HELP, ":%s NOTICE %s :Admin: %s.\n", botname,nick, admin);
  dprintf(DP_HELP, ":%s NOTICE %s :OS: %s %s.\n", botname,nick, uni_t, vers_t);
  dprintf(DP_HELP, ":%s NOTICE %s :Online as: %s!%s.\n", botname,nick, botname, botuserhost);
...
static int msg_die(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  dprintf(-serv, ":%s NOTICE %s :%s\n", botname,nick, BOT_MSGDIE);
  if (!par[0])
    egg_snprintf(s, sizeof s, ":%s BOT SHUTDOWN (authorized by %s)", botname,u->handle);
  else
    egg_snprintf(s, sizeof s, ":%s BOT SHUTDOWN (%s: %s)", botname,u->handle, par);
  chatout("*** %s\n", s);
  ...
  egg_snprintf(s, sizeof s, ":%s DEAD BY REQUEST OF %s!%s", botname,nick, host);
  fatal(s, 0);
  return 1;
}
...
static int msg_save(char *nick, char *host, struct userrec *u, char *par)
{
  ...
  if (u_pass_match(u, par)) {
    ...
    dprintf(DP_HELP, ":%s NOTICE %s :Saving user file...\n", botname,nick);
    write_userfile(-1);
    return 1;
  }
